...
module tb;
    ...
    task one_div_operation(input [31:0] a, b);
        begin
            ...
            expected_quo = a / b;
            expected_rem = a % b;
            $display("Start Division: %0d / %0d", a, b);
            ...
            if (ack) begin
                $display("Expected: Quotient = %0d, Remainder = %0d", expected_quo, expected_rem);
                $display("Received: Quotient = %0d, Remainder = %0d", quo, rem);
                if (quo == expected_quo && rem == expected_rem)
                    $display("✅ RESULT OK");
                else begin
    ...
    initial begin
        @(negedge rst);
        one_div_operation(625, 5);  // ✅ Test case changed to 625 / 5
        #(`PERIOD * 5);
